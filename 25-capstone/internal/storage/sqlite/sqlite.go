...
func (s *Sqlite) GetStudentById(id int64) (types.Student, error) {
	...
	// should be same as DB order of columns here if SELECT *
	// else should be as in SELECT
...
func (s *Sqlite) GetStudents() ([]types.Student, error) {
	...
	for rows.Next() {
		var student types.Student
		err := rows.Scan(&student.Id, &student.Name, &student.Age, &student.Email) // order should be maintained
		if err != nil {
			return nil, err
		}
